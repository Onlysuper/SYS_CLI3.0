<template>
  <!-- 商户管理 -->
  <div class="admin-page customerproduc-page">
    ...
    <el-dialog v-dialogDrag title="配置" center :visible.sync="styleVisible">
      <el-form size="small" :model="styleForm" ref="styleForm" :rules="styleFormRules" label-width="100px">
        ...
        <el-form-item v-if="$store.state.userInfoAndMenu.isAdmin||$store.state.userInfoAndMenu.isOnlyAgent" label="支付类型:" prop="" :label-width="formLabelWidth">
...
export default {
  ...
  methods: {
    ...
    styleFormSave(formName) {
      let splitTime = "2018/04/01";
      let createTime = this.resaultData.createTime;
      if (createTime) {
        createTime = createTime.replace(/-/g, "/")
      }
      let isOnlyAgent = this.$store.state.userInfoAndMenu.isOnlyAgent;
      let timeAgo = new Date(createTime).getTime() < new Date(splitTime).getTime();
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (isOnlyAgent && timeAgo && (invoiceType == 0)) {
            // this.$message({
            //   message: "您的操作权限不可将开票类型设置为空哦！",
            //   type: "warning",
            //   center: true
            // });
            return false;
