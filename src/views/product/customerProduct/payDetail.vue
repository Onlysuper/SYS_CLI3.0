          ...
          <scroll-pane class='tags-view-wrapper' ref='scrollPane'>
            <div class="imgs-group">
              <div class="img-box" v-for="(item,index) in imgsArr" :key="index">
                ...
                <div class="img-back">
                  <img @click="showImg(index,'carousel')" class="img-size" :src="item[1].url" alt="">
                </div>
              </div>
            </div>
          </scroll-pane>
      ...
      getCustomerEchoProduct()({
        ...
        if (res.code == "00") {
          ...
          if (data.imgs) {
            ...
            let newimgsObj = {};// 图片重新排序
            for (let [key, value] of Object.entries(this.imgModal)) {
              let imgName = key;
              if (imgsRow[imgName]) {
                newimgsObj[imgName] = imgsRow[imgName];
                if (!newimgsObj[imgName].imgname) {
                  newimgsObj[imgName].imgname = value.imgname
                }
              }
              // else { // 当图片没有的时候不需要显示，如需要显示则放开注释
              //   newimgsObj[imgName] = value
              // }
            }
            let imgsArr = Object.entries(newimgsObj);
