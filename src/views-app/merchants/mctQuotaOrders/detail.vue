  ...
  <full-page>
    ...
    <view-radius>
      <input-wrapper>
        ...
        <mt-cell title="单笔交易限额(分)">
          <span>{{dataList.singleAmountLimit}}</span>
        </mt-cell>
        <mt-cell title="日交易限额(分)">
          <span>{{dataList.dayAmountLimit}}</span>
        </mt-cell>
        <mt-cell title="当前额度(分)">
          <span>{{dataList.currentAmount}}</span>
        </mt-cell>
        <mt-cell title="状态">
          <span>
            {{dataList.status | statusFilter("mctQuotaOrdersStatus")}}
          </span>
        </mt-cell>
      </input-wrapper>
    </view-radius>
...
export default {
  ...
  methods: {
    ...mapActions(["getMctQuotaOrders"]),
    init() {
      if (this.pageType == "DETAIL") {
        this.surePage = false;
        this.getMctQuotaOrders({
