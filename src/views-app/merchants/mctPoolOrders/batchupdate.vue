<template>
  <full-page class="page">
    ...
    <view-radius class="split-box">
      <input-wrapper>
        <mt-field :disabled="customerNoDis" :readonly="customerNoDis" label="商户编号" placeholder="请输入商户编号，用空格/逗号隔开" v-model="addForm.customerNo" type="textarea" rows="4"></mt-field>
        <mt-field @click.native="$refs['bussinessTypePicker'].open()" type="text" label="经营类型" placeholder="请选择经营类型" :value="bussinessTypeModle.name" v-readonly-ios :disableClear="true">
...
export default {
  ...
  methods: {
    ...mapActions(["getPoolOrders", "postEditMctPoolOrder", "postAddMctPoolOrder", "postBatchUpdateMctPoolOrder"]),
    //地区选择回调函数
    resultCallback(obj) {
      this.city = obj;
    ...
    save() {
      ...
      if (this.pageType == "UPDATE") {
        ...
        let updateData = Object.assign({}, utils.pickObj(this.addForm, ["status", "bussinessType"]), { customerNos: customerNos })
        this.postBatchUpdateMctPoolOrder({ ...updateData }).then(flag => {
          this.btnDisabled = false;
          if (flag) {
            this.$store.commit("MCTPOOLORDERS_QUERY_IS_SEARCH", true);
            this.$router.back();
