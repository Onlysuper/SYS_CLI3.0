    ...
    <full-page class="page" ref="FullPage">
      ...
      <myp-loadmore-api class="list" ref="MypLoadmoreApi" :api="api" @watchDataList="watchDataList">
        <myp-cell-pannel class="spacing-20" v-for="(item,index) in list" :key="index" :title="item.dataTime">
          ...
          <!-- <mt-badge v-if="item.status=='BINDED'" slot="badge" class="g-min-badge" size="small" type="success" :color="filterColor(item.status,'empowerBindStatus')">{{item.status | statusFilter('empowerBindStatus')}}</mt-badge> -->
          <mt-badge slot="badge" class="g-min-badge" size="small" type="success" :color="filterColor(item.status,'empowerBindStatus')">{{item.status | statusFilter('empowerBindStatus')}}</mt-badge>
          <mt-badge slot="badge" class="g-min-badge" size="small" type="primary" :color="filterColor(item.materiel,'empowerManageMateriel')">{{item.materiel | statusFilter('empowerManageMateriel')}}</mt-badge>
          <mt-badge slot="badge" class="g-min-badge" size="small" type="primary" :color="filterColor(item.deviceType,'empowerCheckReceiptType')">{{item.deviceType | statusFilter('empowerCheckReceiptType')}}</mt-badge>
          <!-- 常用按钮 -->
