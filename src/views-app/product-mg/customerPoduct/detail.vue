<template>
  <full-page class="customerpro-detail-page">
    <mt-header slot="header" :title="$route.meta.pageTitle" class="re-mint-header">
      ...
      <mt-button v-if="elecCheckButVisible" slot="right" :disabled="false" type="danger" @click="popupActionsVisible = !popupActionsVisible">...</mt-button>
    </mt-header>
    <!-- actions操作 -->
    ...
    <template v-if="productType == 'pay'">
      ...
      <view-radius class="uploads imagesParent" id="WeixinJSBridge">
        ...
        <!-- 美食 -->
        <show-img-view v-show="category=='food'" class="item" :disabled="true" ref="wechatLogoCashSpaceImg" :label="'微信标示前台照'"></show-img-view>
        <show-img-view v-show="category=='food'" class="item" :disabled="true" ref="alipayLogoCashSpaceImg" :label="'支付宝标示前台照'"></show-img-view>
        <show-img-view v-show="category=='food'" class="item" :disabled="true" ref="caterimgPlatformCertificateImg" :label="'三方平台凭证'"></show-img-view>
        <show-img-view v-show="category=='food'" class="item" :disabled="true" ref="providerStoredoorImg" :label="'BD与门头合影'"></show-img-view>
        <show-img-view v-show="category=='food'" class="item" :disabled="true" ref="providerWechatScanImg" :label="'BD与微信所有物料'"></show-img-view>
        <show-img-view v-show="category=='food'" class="item" :disabled="true" ref="providerAlipayScanImg" :label="'BD与支付宝所有物料'"></show-img-view>
        <show-img-view v-show="category=='food'" class="item" :disabled="true" ref="caterimgPermitImg" :label="'餐饮许可证'"></show-img-view>
      </view-radius>
    </template>
    <!-- 电票详情 -->
    <template v-if="productType == 'elec'">
      <view-radius>
        <input-wrapper>
          ...
          <p>
            {{$store.state.userInfoAndMenu.isOperate}}
          </p>
          <p>
            {{elec.elecStatus}}
          </p> -->
...
<script>
import { getCustomerEchoProduct, getQueryCustomerElectronic, postHandleCustomerProduct } from "@src/apis";
...
export default {
  ...
  methods: {
    ...
    echoFormElec(data) {
      ...
      if (product instanceof Object) {
        this.elec.elecStatus = product.elecStatus;
        this.elec.mounthCount = product.elecBillnum;
      }

      if (this.productType == 'elec' && this.$store.state.userInfoAndMenu.isOperate &&
        (this.elec.elecStatus == 'CHECKING' || this.elec.elecStatus == 'PROCESSING')) {
    ...
    checkFn(type) {
      ...
      let obj = {
        bussinessNo: customerNo,
        bussinessType: "customer",
      };
      if (type == 'adopt') {
        this.MessageBox.confirm('确定要通过审核吗?').then(action => {
          obj.elecStatus = "TRUE";
          this.resaultHandle(obj, 'eleCheck');
        });
        // 审核通过
      } else if (type == 'refuse') {
        this.MessageBox.prompt(' ', '请填写拒绝原因').then(({ value }) => {
          if (value) {
            // 审核拒绝
        ...
        this.MessageBox.prompt(' ', '请填写关闭原因').then(({ value }) => {
          if (value) {
            // 审核拒绝
            obj.elecStatus = "FALSE";
            obj.elecReason = value;
            this.resaultHandle(obj, 'eleClose');
          }
        });
      }
    },
    resaultHandle(obj, type) {
      console.log(type);
      postHandleCustomerProduct()(obj).then(res => {
        if (res.code == "00") {
          this.Toast("操作成功");
          // this.elecCheckButVisible = false;
          // this.init();
          this.$store.commit("CUSTOMER_PRODUCT_IS_SEARCH", true);
          if (type == 'eleCheck') {
            this.popupActions = this.popupActions.filter((item) => {
              if (item.name != "审核通过" && item.name != "审核拒绝") {
                return item
              }
            })
          }
