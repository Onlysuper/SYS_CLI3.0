      ...
      <div class="add-playinfo">
        <view-radius>
          <input-wrapper>
            ...
            <mt-field label="法人:" @change="legalPersonChangeHandle" type="text" v-model="form.legalPerson" placeholder="输入法人姓名" v-required :attr="{maxlength:50}"></mt-field>
            ...
            <mt-radio @change="accountTypeChangeHandle" title="结算信息" v-model="form.accountType" :options="[{ label: '对公',value: '0' },{ label: '对私',value: '1' }]" class="mint-radiolist-row border-1px"></mt-radio>
...
export default {
  ...
  methods: {
    ...
    // 结算人身份证号显示隐藏
    isLegalPersonSettleIdCard(type, val) {
      let accountType = val || this.form.accountType;
      if (accountType == "0") {
        // 对公
    ...
    echoForm(data) {
      ...
      if (settleCard instanceof Object) {
        ...
        this.form.accountName = settleCard.accountName;
        ...
        this.getaccountName = settleCard.accountName;
      }
      this.isLegalPersonSettleIdCard();
    },
    //处理对公对私逻辑
    ...
    accountNameChange(val) {
      let accountType = val || this.form.accountType
      if (accountType == "0") {
        this.form.accountName = this.enterpriseName
      }
      else {
        this.form.accountName = this.getaccountName == this.enterpriseName ? this.form.legalPerson : this.getaccountName || this.form.legalPerson      };
    },
    // 结算信息切换
    accountTypeChangeHandle(val) {
      this.accountNameChange(val);
      this.isLegalPersonSettleIdCard(null, val);
      this.cacheFrom(); //缓存数据
    },
    // 法人姓名改变
    legalPersonChangeHandle() {
