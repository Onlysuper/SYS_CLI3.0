<template>
  ...
  <div class="tablelist-box">
    ...
    <el-table ref="tableList" type="index" border :fit="true" :stripe="true" :data="tableData" class="__scrollStyle__" height="100%" max-height="100%" width="100%" v-loading="ifloading" empty-text="暂无数据" header-row-class-name="tableHeader" show-overflow-tooltip="true">
      ...
      <el-table-column v-for="(item,index) in tableDataInit.dataHeader" :key="index" :prop="item.word" :label="item.key" v-if="item.hidden?false:true" :min-width="item.width" :sortable="item.sortable">
        <template slot-scope="scope" v-if="item.visibleFn?item.visibleFn(scope.row):true">
          <el-tag size="mini" v-if="item.status&&item.type(scope.row[scope.column.property],scope.row).text!='undifined'&&scope.row[scope.column.property]!=null&&scope.row[scope.column.property]!=''?true:false" :style="item.type(scope.row[scope.column.property],scope.row).color?'color:#fff':''" :type="item.type(scope.row[scope.column.property],scope.row).type?item.type(scope.row[scope.column.property],scope.row).type:''" :color="item.type(scope.row[scope.column.property],scope.row).color?item.type(scope.row[scope.column.property],scope.row).color:''" close-transition disable-transitions> {{scope.row[scope.column.property]!='null'?item.type(scope.row[scope.column.property],scope.row).text:""}}</el-tag>
          <span class="link-text inline-text" v-else-if="item.event&&scope.row[scope.column.property]!='null'&&scope.row[scope.column.property]!='undifined'?true:false" @click="operationHandle(scope.row,item.cb)">
          ...
          <el-popover v-else-if="scope.row[scope.column.property]!='undifined'?true:false" trigger="click" placement="top">
            <p>
      ...
      <el-table-column align="left" v-if="tableDataInit.operation" fixed="right" label="操作" :width="tableDataInit.operation.width">
        <template slot-scope="scope">
          <!-- <el-button-group> -->
          <el-button type="text" class="allButton" :icon="buticon(item.text)" v-for="(item,index) in tableDataInit.operation.options" :ref="item.ref" :privilege-code="item.ref" :key="index" size="mini" v-if="item.visibleFn?item.visibleFn(scope.row,item.visibleFn):true" :disabled="item.disabledFn?item.disabledFn(scope.row,item.disabledFn):false" :loading="operationLoading[scope.$index]" @click="operationHandle(scope.row,item.cb,scope.$index,item.loading)" :style="item.color?'color:'+item.color:'color:#1890ff,background:red'">
            {{buticon(item.text)?"":item.text}}
            <!-- {{item.text}} -->
          </el-button>
          <!-- </el-button-group> -->
        </template>
...
export default {
  ...
  methods: {
    ...
    postDataInit(page, limit, searchCondition) {
      ...
      this.actionUrl()({
        ...
        if (data.code === "00") {
          ...
          if (this.addKeys && this.addKeys.length > 0) {
            for (var i = 0; i < this.tableData.length; i++) {
              this.addKeys.forEach(element => {
                this.$set(this.tableData[i], element.word, element.val)
              });
            }
          }
    ...
    operationHandle(rowdata, cb, index, loading) {
      // 点击操作按钮
      if (loading) {
        this.$set(this.operationLoading, index + '', true);
      }
      this.$emit("operation", rowdata, cb, this.operationLoading, index);
    },
    doLayoutReload() {
      this.$nextTick(() => {
  ...
  computed: {
    ...
    addKeys() {
      return this.tableDataInit.addKeys;
    },
    // visibleinput() {
  ...
  watch: {
    visibleinput(val) {
      this.doLayoutReload();
      console.log("visibleinput:刷新");
      // 监听高级搜索与普通搜索模式转变
    },
    getPage(value) {
      this.getPage = value;
      this.postDataInit(this.getPage, this.getLimit, this.getSearch);
      console.log("getPage:刷新");
    },
    limit(value) {
      this.getLimit = value;
      this.postDataInit(this.getPage, this.getLimit, this.getSearch);
      console.log("limit:刷新");
    },
    // getUrl(value) {
    //   this.getUrl = value;
    //   this.postDataInit(this.getPage, this.getLimit, this.getSearch);
    ...
    getSearch() {
      ...
      this.postDataInit(this.getPage, this.getLimit, this.getSearch);
      console.log("getSearch:刷新");
    },
    actionUrl() {
      this.postDataInit(this.getPage, this.getLimit, this.getSearch);
      console.log("actionUrl:刷新");
    },
    fullScreen(value) {
      // 全屏切换
    },
